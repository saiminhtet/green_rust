...
impl Glyph {
    ...
    fn new_random<R: Rng>(rand: &mut R, color: Color) -> Self {
        ...
        Self{
            ...
            character: characters.chars().nth(rand.gen_range(0..characters.chars().count())).unwrap(),
            color,
        }
    }

    fn empty() -> Self {
        Self { character: ' ', color: Color::from_rgb(0, 0, 0), }
    }
    fn render<W: Write>(&self, out: &mut W) -> Result<()> {
        queue!(out, style::SetBackgroundColor(style::Color::Rgb { r: 0, g: 0, b: 0 }))?;
        queue!(out, style::SetForegroundColor(style::Color::Rgb { r: self.color.r, g: self.color.g, b: self.color.b }))?;
    ...
    fn fade_color(&mut self) {
        ...
        if new_color.s < 20.0 || new_color.l < 20.0 {
             self.color = HslColor::new(hsl.h, 20.0, 20.0).into();
        } 
        else {
...
fn main() -> Result<()>{
    ...
    let seed = SystemTime::now()
        ...
        .expect("time to have pass since UNIX_EPOCH")
        .as_micros() as u64;
